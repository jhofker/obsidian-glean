...
import { testGleanConnection } from "./glean-client";
...
function createInstanceNameSetting(
...
function createCorsMethodSetting(
  containerEl: HTMLElement,
  settings: GleanSettings,
  onSave: () => Promise<void>
): void {
  new Setting(containerEl)
    ...
    .addToggle((toggle) =>
      toggle.setValue(settings.useCorsFreeMethods).onChange(async (value) => {
